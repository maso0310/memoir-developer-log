...
// 生成時間: 2025-09-16T03:44:32.452672400+00:00
...
// ========== 提取的腳本區塊 ==========
        ...
        // 版本識別系統 - 檢查是否需要重設使用者設定
        const CURRENT_VERSION = '1757994272';
        const STORED_VERSION = localStorage.getItem('memoirflow:version');

        // 如果版本不同，清除快取的使用者設定
        if (STORED_VERSION !== CURRENT_VERSION) {
            console.log('🔄 檢測到新版本 (' + CURRENT_VERSION + ')，清除快取設定');

            // 清除使用者自訂的設定
            const keysToRemove = [
                'memoirflow:typing-speed',
                'memoir-theme',
                'memoirflow:font-size',
                'memoirflow:thumbnails-visible',
                'memoirflow:typewriter-enabled',
                'memoirflow:controls-hidden',
                'memoirflow:date-hidden'
            ];
            ...
            // 儲存新版本號
            localStorage.setItem('memoirflow:version', CURRENT_VERSION);
            console.log('✅ 設定重設完成，將使用預設值');
        } else {
            console.log('📦 版本相同 (' + CURRENT_VERSION + ')，保留現有設定');
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (typeof window.REQUIRE_PW !== 'undefined' && window.REQUIRE_PW && !sessionStorage.getItem('mf_pw_unlocked')) {
                ...
                // 嘗試從 preloaded-memoir-data 元素載入數據
                console.log('🔍 嘗試從 JSON 元素載入數據...');
                const dataElement = document.getElementById('preloaded-memoir-data');
                if (dataElement) {
                    try {
                        const jsonText = dataElement.textContent || dataElement.innerText;
                        const parsedData = JSON.parse(jsonText);
                        window.MEMOIR_DATA = parsedData;
                        MEMOIR_DATA = parsedData;
                        console.log('✅ 從 JSON 元素成功載入數據');
                        initializeApp();
                    } catch (e) {
                        console.error('❌ 解析 JSON 數據失敗:', e);
                        console.log('⏳ 等待數據載入...');
                    }
                } else {
                    console.log('⏳ 等待數據載入...');
