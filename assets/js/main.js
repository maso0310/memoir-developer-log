...
// 生成時間: 2025-09-17T09:39:38.836557500+00:00
...
// ========== 提取的腳本區塊 ==========
            ...
            window._internalAutoDecrypt = async function(passwordInput) {
                ...
                try {
                    ...
                    console.log('🔍 encodedData 存在:', !!encodedData);
                    console.log('🔍 expectedPasswordHash 存在:', !!expectedPasswordHash);
                    console.log('🔍 expectedPasswordHash 長度:', expectedPasswordHash ? expectedPasswordHash.length : 0);

                    if (encodedData && expectedPasswordHash) {
                        const calculatedHash = await hashPassword(password);
                        console.log('🔐 輸入的密碼:', password);
                        console.log('🔐 鹽值附加後:', password + '_memoir_salt_2024');
                        console.log('🔐 計算出的哈希 (長度 ' + calculatedHash.length + '):', calculatedHash);
                        console.log('🔐 期望的哈希 (長度 ' + expectedPasswordHash.length + '):', expectedPasswordHash);

                        // 字符級比較分析
                        let mismatchCount = 0;
                        for (let i = 0; i < Math.max(calculatedHash.length, expectedPasswordHash.length); i++) {
                            if (calculatedHash[i] !== expectedPasswordHash[i]) {
                                mismatchCount++;
                                if (mismatchCount <= 5) { // 只顯示前5個差異
                                    console.log('🔐 差異位置 ' + i + ':', {
                                        計算: calculatedHash[i] || 'undefined',
                                        期望: expectedPasswordHash[i] || 'undefined'
                                    });
                                }
                            }
                        }
                        console.log('🔐 總差異數量:', mismatchCount);
                        console.log('🔐 哈希是否匹配:', calculatedHash === expectedPasswordHash);
